...
def checkout():
    ...
    total = sum(item['subtotal'] for item in cart)
    pelanggans = Pelanggan.query.all()
    return render_template('pembayaran.html', cart=cart, total=total, pelanggans=pelanggans)
...
def proses_pembayaran():
    ...
    potongan_input = request.form.get('potongan', '0')
    if not potongan_input: potongan_input = '0' 
    potongan = float(potongan_input)
    
    uang_bayar_input = request.form.get('uang_bayar', '0')
    if not uang_bayar_input: uang_bayar_input = '0'
    uang_bayar = float(uang_bayar_input)
    
    total_akhir = total_belanja - potongan
    kembalian = uang_bayar - total_akhir
    hutang_baru = 0 # Inisialisasi hutang
    
    # --- LOGIKA HUTANG ---
    pelanggan_id = request.form.get('pelanggan_id')
    if kembalian < 0:
        if not pelanggan_id:
            flash('Uang kurang! Jika ingin berhutang, wajib memilih nama Pelanggan di bawah.', 'danger')
    ...
    # Uang yang benar-benar masuk laci hari ini
    uang_masuk_nyata = total_akhir - hutang_baru

    # Simpan uang masuk nyata sebagai total_penjualan
    ...
    for item in cart:
        ...
        if prod:
            ...
            modal_now = prod.modal
            total_modal += (modal_now * item['quantity'])
            db.session.add(SaleItem(sale_id=new_sale.id, product_name=item['name'], quantity=item['quantity'], price=item['price'], modal=modal_now, sale_type=item['price_type'], subtotal=item['subtotal']))
            list_barang_str += f"▪️ {item['name']} ({item['quantity']}x) @ {item['price']:,.0f}\n"

    # Keuntungan = Uang nyata yang diterima - Total modal barang
    new_sale.keuntungan = uang_masuk_nyata - total_modal
    db.session.commit()
    
    try: 
        pesan_telegram = (f"🛒 <b>TRANSAKSI ({session.get('role', 'user').upper()})</b>\n\n📅 {datetime.now().strftime('%d/%m %H:%M')}\n----------------\n{list_barang_str}----------------\n💰 Harga Total: Rp {total_akhir:,.0f}\n💳 Dibayar Tunai: Rp {uang_masuk_nyata:,.0f}\n📒 Sisa (Kasbon): Rp {hutang_baru:,.0f}")
        send_telegram_message(pesan_telegram)
    except: pass
    
    session.pop('cart', None)
    return render_template('struk.html', cart=cart, total=total_belanja, potongan=potongan, total_akhir=total_akhir, uang_bayar=uang_bayar, kembalian=kembalian)
...
def bayar_hutang():
    ...
    if pelanggan_id and jumlah_bayar > 0:
        pelanggan = Pelanggan.query.get(pelanggan_id)
        if pelanggan:
            # Mencegah kasir menginput uang lebih besar dari total hutang
            if jumlah_bayar > pelanggan.total_hutang:
                jumlah_bayar = pelanggan.total_hutang
                
            if jumlah_bayar >= pelanggan.total_hutang:
                pelanggan.total_hutang = 0 # Lunas
                flash(f'Hutang {pelanggan.nama} telah LUNAS!', 'success')
            else:
                pelanggan.total_hutang -= jumlah_bayar
                flash(f'Pembayaran hutang {pelanggan.nama} sebesar Rp {jumlah_bayar:,.0f} berhasil dicatat. Sisa hutang: Rp {pelanggan.total_hutang:,.0f}', 'success')
            
            # --- CATAT PEMBAYARAN HUTANG SEBAGAI PEMASUKAN DI KEUANGAN ---
            # Karena modal barang sudah dikurangi saat transaksi pertama, uang cicilan hutang ini 100% adalah keuntungan (uang masuk murni)
            new_payment = Sale(total_penjualan=jumlah_bayar, total_potongan=0, keuntungan=jumlah_bayar)
            db.session.add(new_payment)
            db.session.flush()
            
            # Tambah keterangan di riwayat barang agar jelas asal uangnya
...
def keuangan():
    ...
    ops_month = db.session.query(db.func.sum(OperationalCost.jumlah)).filter(db.extract('month', OperationalCost.tanggal) == current_month).scalar() or 0
    net_profit = gross_profit_month - ops_month
    return render_template('keuangan.html', gross_profit=gross_profit_month, ops_cost=ops_month, net_profit=net_profit, recent_sales=Sale.query.order_by(Sale.timestamp.desc()).limit(10).all())
